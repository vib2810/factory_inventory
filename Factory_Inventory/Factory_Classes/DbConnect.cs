...
namespace Factory_Inventory.Factory_Classes
{
    public class DbConnect
    {
        ...
        public bool addSalesVoucher(DateTime dtinput, DateTime dtissue, string type, string quality, string company, string customer, List<Tuple<string, string>> cartonno, float sell_cost, string sale_do_no, string tablename, float net_weight)
        {
            ...
            {
                con.Open();
                string sql = "INSERT INTO Sales_Voucher (Date_Of_Input,Date_Of_Sale, Quality, Company_Name, Customer, Sale_Rate, Fiscal_Year, Type_Of_Sale, Tablename, Sale_DO_No, Net_Weight, Printed, Payment_Closed) VALUES ('" + inputDate + "' ,'" + issueDate + "','" + quality + "', '" + company + "', '" + customer + "', " + sell_cost + " , '" + fiscal_year + "', '" + int.Parse(type) + "', '" + tablename + "', '" + sale_do_no + "', " + net_weight + ", 0, 0); SELECT SCOPE_IDENTITY()";
                Console.WriteLine(sql);
                SqlDataAdapter adapter = new SqlDataAdapter(sql, con);
