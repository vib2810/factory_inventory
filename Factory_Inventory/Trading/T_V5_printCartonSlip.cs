...
{
    public partial class T_V5_printCartonSlip : Form
    {
        private DbConnect c;
        ...
        List<string> cartons_id_to_print = new List<string>();
        ...
        public T_V5_printCartonSlip()
        {
            ...
            //Datagridviews
            string sql = "SELECT temp3.*,\n";
	        sql+="  (SELECT SUM(T_Inward_Carton.Net_Weight)\n";
            sql += "    FROM T_Inward_Carton\n";
            sql += "    WHERE T_Inward_Carton.Repacking_Voucher_ID = temp3.Voucher_ID) Net_Weight\n";
            sql += "    FROM (SELECT temp2.*, T_M_Company_Names.Company_Name\n";
            sql += "        FROM(SELECT temp1.*, T_M_Quality_Before_Job.Quality_Before_Job\n";
            sql += "            FROM(SELECT T_Repacking_Voucher.*, T_M_Colours.Colour\n";
            sql += "            FROM T_Repacking_Voucher\n";
            sql += "            LEFT OUTER JOIN T_M_Colours ON T_M_Colours.Colour_ID = T_Repacking_Voucher.Colour_ID) as temp1\n";
            sql += "        LEFT OUTER JOIN T_M_Quality_Before_Job ON temp1.Quality_ID = T_M_Quality_Before_Job.Quality_Before_Job_ID) as temp2\n";
            sql += "    LEFT OUTER JOIN T_M_Company_Names ON temp2.Company_ID = T_M_Company_Names.Company_ID) as temp3\n";
            sql += "WHERE temp3.Deleted IS NULL\n";
            sql += "ORDER BY Voucher_ID DESC\n";

            DataTable dyeing_batches = c.runQuery(sql);
            ...
            if (true)
            {
                ...
                this.dataGridView1.Columns["Start_Date_Of_Production"].DisplayIndex = i++;
                this.dataGridView1.Columns["Date_Of_Production"].Visible = true;
                this.dataGridView1.Columns["Date_Of_Production"].DisplayIndex = i++;
                this.dataGridView1.Columns["Colour"].Visible = true;
                this.dataGridView1.Columns["Colour"].DisplayIndex = i++;
                ...
                this.dataGridView1.Columns["Fiscal_Year"].DisplayIndex = i++;
            ...
            this.dt4.Columns.Add("Carton ID");
            ...
            dataGridView4.Columns["Carton ID"].Visible = false;
            ...
            this.dataGridView5.Columns.Add("Carton ID", "Carton ID");
            ...
            dataGridView5.Columns["Carton ID"].Visible = false;
        ...
        private void load_batch()
        {
            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                voucher = (dataGridView1.Rows[index].DataBoundItem as DataRowView).Row;
                voucher_id = int.Parse(voucher["Voucher_ID"].ToString());
            }
            if (dataGridView2.SelectedRows.Count > 0)
            {
                ...
                voucher = (dataGridView2.Rows[index].DataBoundItem as DataRowView).Row;
                voucher_id = int.Parse(voucher["Voucher_ID"].ToString());
            }
            if (voucher_id == -1)
            {
            ...
            //dt4
            this.dt4.Clear();
            this.carton_fiscal_year = voucher["Carton_Fiscal_Year"].ToString(); //set carton_fiscal_year

            string sql = "SELECT Carton_ID, Carton_No, Net_Weight, Printed FROM T_Repacked_Cartons WHERE Repacking_Voucher_ID = " + voucher_id;
            DataTable carton_info = c.runQuery(sql);

            for (int i = 0; i < carton_info.Rows.Count; i++)
            {
                string carton_id = carton_info.Rows[i]["Carton_ID"].ToString();
                string carton_no = carton_info.Rows[i]["Carton_No"].ToString();
                float net_wt = float.Parse(carton_info.Rows[i]["Net_Weight"].ToString());
                dt4.Rows.Add(i + 1, carton_no, net_wt, this.carton_fiscal_year, carton_id);
                dataGridView4.DataSource = this.dt4;
                if (ispresent(dataGridView5, carton_id) >= 0)
                {
        ...
        int ispresent(DataGridView d, string carton_id)
        {
            for (int i = 0; i < d.Rows.Count; i++)
            {
                if (d.Rows[i].Cells["Carton ID"].Value.ToString() == carton_id)
                {
                    return i;
                }
            }
        ...
        private void load_cartons()
        {
            for (int i = 0; i < dataGridView4.Rows.Count; i++)
            {
                ...
                if (ispresent(dataGridView5, row.Cells["Carton ID"].Value.ToString()) == -1)
                {
                    this.dataGridView5.Rows.Add("", row.Cells[1].Value, row.Cells[2].Value, row.Cells[3].Value, row.Cells["Carton ID"].Value);
        ...
        private void printButton_Click(object sender, EventArgs e)
        {
            ...
            this.cartons_id_to_print.Clear();
            int rows = dataGridView5.Rows.Count;
            string sql = "begin transaction; begin try;\n";
            for (int i = 0; i < dataGridView5.RowCount; i++)
            {
                string carton_id = dataGridView5.Rows[i].Cells["Carton ID"].Value.ToString();
                this.cartons_id_to_print.Add(carton_id);
                sql += "UPDATE T_Repacked_Cartons SET Printed=1 WHERE Carton_ID = '" + carton_id + "';\n";
            }
            ...
            this.slip_count = cartons_id_to_print.Count;
        ...
        private void deleteToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                int index = ispresent(dataGridView4, row.Cells["Carton ID"].Value.ToString());
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            {
                write_slip(e, w[local_print_count], running_y, slip_width, slip_height, this.cartons_id_to_print[this.printed_slips]);
        ...
        int write_slip(System.Drawing.Printing.PrintPageEventArgs e, int x, int write_height, int width, int height, string carton_id)
        {
            ...
            sql += "        LEFT OUTER JOIN T_M_Quality_Before_Job ON temp1.Quality_ID = T_M_Quality_Before_Job.Quality_Before_Job_ID) temp2\n";
            sql += "    LEFT OUTER JOIN T_Repacking_Voucher ON temp2.Repacking_Voucher_ID = T_Repacking_Voucher.Voucher_ID) temp3\n";
            sql += "LEFT OUTER JOIN T_M_Cones ON temp3.Cone_ID = T_M_Cones.Cone_ID\n";
            sql += "WHERE Carton_ID = '" + carton_id + "'\n";
            DataTable dt = c.runQuery(sql);
            DataRow carton_data = dt.Rows[0];
            ...
            if (this.printFormatCB.SelectedItem.ToString()=="1")
            {
                ...
                write(e, x + (int)(0.19 * width), write_height - 1, (int)(0.51 * width), ": " + carton_data["Quality_Before_Job"].ToString(), basic_size + 4, 'l', 1, 0);
                ...
                string voucher_id = carton_data["Repacking_Voucher_ID"].ToString();
                write(e, x + (int)(0.02 * width), write_height, (int)(0.23 * width), "Batch No", basic_size + 3, 'l', 0, 0);
                write_height += write(e, x + (int)(0.25 * width), write_height, (int)(0.73 * width), ":  " + voucher_id, basic_size + 3, 'l', 0, 0) + gap;
                e.Graphics.DrawLine(new Pen(Color.Black, 1), x, write_height, x + width, write_height);
                ...
                string barcode_data = voucher_id;
                ...
                string tare_wt = (float.Parse(carton_data["Carton_Weight"].ToString()) + int.Parse(carton_data["Number_Of_Cones"].ToString()) * float.Parse(carton_data["Cone_Weight"].ToString()) / 1000F).ToString("F3");
            ...
            else if(this.printFormatCB.SelectedItem.ToString()=="2")
            {
                ...
                write_height+=write(e, x + (int)(0.26 * width), write_height - 1, (int)(0.70 * width), ": " + carton_data["Quality_Before_Job"].ToString(), basic_size + 4, 'l', 1, 0);
                ...
                write_height += write(e, x + (int)(0.26 * width), write_height, (int)(0.74 * width), ": " + voucher_id, basic_size + 3, 'l', 0, 0);
